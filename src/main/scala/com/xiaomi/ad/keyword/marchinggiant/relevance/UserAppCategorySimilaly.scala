...
object UserAppCategorySimilaly {
  ...
  def cmpCosin(catSeq: Seq[help], adInfoMap: Map[String, Seq[Seq[String]]], tpc: Int): Seq[cosResult1] = {
  ...
  def execute(args: Args, sparkConf: SparkConf) = {
    ...
    val user = spark.read.parquet(args("input1"))
      ...
      .map { m =>
        ...
        val appGoogle = cmpCosin(m.gCatSeq, appAndCateB.value, 0)
        val appEmi = cmpCosin(m.emiCatSeq, appAndCateB.value, 1)
        val appLda = cmpCosin(m.topicSeq, appAndCateB.value, 2)

        cosResult3WithApp(m.imei1Md5, Goole, Emi, Lda, appGoogle, appEmi, appLda)
      }
      .filter(r => r.cosSimG.nonEmpty || r.cosSimE.nonEmpty || r.cosSimL.nonEmpty||r.appCosSimE.nonEmpty||r.appCosSimG.nonEmpty||r.appCosSimL.nonEmpty)
      .repartition(500)
      .write
