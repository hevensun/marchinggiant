...
/**
  * Created by cailiming on 17-8-3.
  */
object ParseBaiduAppQuery {
    ...
    def isValidQueryInfoKeySet(keySet: Set[String]): Boolean = {
        keySet.contains("百科") && keySet.contains("相关搜索") && keySet.contains("广告") && keySet.contains("搜索结果") && keySet.size == 4//keySet.contains("package") &&
    }

    def createErrorQI(errorType: String, queryStr: String): QueryInfo = {
        QueryInfo(errorType + queryStr)
    }

    def getQueryInfo(queryStr: String): QueryInfo = {
        try{
...
//            val packageName = (queryInfo \ "package").asInstanceOf[JString].values

            QueryInfo(query, baike, getSeqFromJson(queryInfo, "相关搜索"), getTextInfoFromJson(queryInfo, "广告"), getTextInfoFromJson(queryInfo, "搜索结果"))
