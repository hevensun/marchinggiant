...
object FindSimilaryApps2 {
    ...
    def execute(args: Array[String], sparkConf: SparkConf): Unit = {
        for (config <- configParser.parse(args, Config(null, 0.0, null, null, null))) {
            ...
            val vectorDf = spark.read.parquet(config.input)
                ...
                .as[EnhancedAppInfo2]
                .map{ ea =>
                    val categoryCategoryMin = Try(ea.categoryCategory.map(_.score).min).getOrElse(0.0)
                    val categoryCategoryMax = Try(ea.categoryCategory.map(_.score).max).getOrElse(0.0)
                    val normalizedCategoryCategory = ea.categoryCategory.map(c => c.copy(catName = null, score = minMaxNormalizer(categoryCategoryMin, categoryCategoryMax, c.score)))

                    val keywordCategoryMin = Try(ea.keywordCategory.map(_.score).min).getOrElse(0.0)
                    val keywordCategoryMax = Try(ea.keywordCategory.map(_.score).max).getOrElse(0.0)
                    val normalizedKeywordCategory = ea.keywordCategory.map(c => c.copy(catName = null, score = minMaxNormalizer(keywordCategoryMin, keywordCategoryMax, c.score)))

                    val introCategoryMin = Try(ea.introductionCategory.map(_.score).min).getOrElse(0.0)
                    val introCategoryMax = Try(ea.introductionCategory.map(_.score).max).getOrElse(0.0)
                    val normalizedIntroCategory = ea.introductionCategory.map(c => c.copy(catName = null, score = minMaxNormalizer(introCategoryMin, introCategoryMax, c.score)))
                    ...
                    val de = lev1C + lev2C + rawK + normalizedCategoryCategory.map(c => Math.pow(c.score, 2)).sum + normalizedKeywordCategory.map(c => Math.pow(c.score, 2)).sum + normalizedIntroCategory.map(c => Math.pow(c.score, 2)).sum + normalizedLda.map(t => Math.pow(t.score, 2)).sum
                    ...
                    AppVectorWithName2(ea.packageName, ea.displayName, Seq(), rawKeyword, lev1Name, lev2Name, norCategoryCategory, norIntroCateogry, norKeywordCategory, norTopic)
                }
    ...
    def similarity(v1: AppVectorWithName2, v2: AppVectorWithName2) = {
        ...
        val numerator = lev1Num + lev2Num + rawKeywordNum + cateCateNum + introCateNum + keywordCateNum + topicNum
        numerator
    }
}
